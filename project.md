...
- 115 年的新考題已經挖出來了，答案也核對過是對的，但每題都缺「分類標籤」和
  「為什麼答案是這個」的解析。
- 沒寫完的題目程式不會放進題庫，所以**使用者現在打開 app 一題都看不到**。
- 這是唯一擋住新題上線的事。100 題，建議先做科一 50 題給使用者看品質，再做科三。

### 解析寫完之後的收尾（§I）

**3 件是「按按鈕確認一下」：**

1. （I1）重新產生網頁檔，題庫從 795 變 895 題
2. （I2）跑檢查指令確認沒問題
3. （I3）自己打開網頁看看：多的 100 題在不在、圖表題的圖有沒有顯示出來、
   點「看原始 PDF」跳不跳得到正確頁

**2 件是「改文件」：**

4. （I4）**README 要改** — 上面還寫 795 題，而且完全沒提到新寫的工具程式。
   這件最重要：不寫的話，以後誰要改題庫都不知道有工具，又會回去手改網頁檔，
   就是之前出錯的那個坑。
5. （I5）project.md 裡的數字更新一下

### 兩件之前漏掉的（複查時補進 checklist）

6. （I6）**裝一個自動檢查** — 之前出過一次錯：有人改了題庫但忘記重新產生網頁，
   使用者三個月都在練舊題沒人發現。這次寫了檢查指令可以抓出來，但**沒設定成
   自動跑**，還是要人記得。所以同樣的錯還會再犯。要設定成「每次 commit 前
   自動檢查，不對就擋下來」。
7. （I7）**查一下今年官方有沒有出新的樣題** — 題庫裡的 45 題樣題是去年 9 月版的。
   查完不管有沒有，都把這件事結掉，不要一直懸著。

### 順序建議

先做 **6（裝防呆）** 跟 **4（改 README）**——這兩個現在就能做，五分鐘的事，
而且是在防「下次又出錯」。**7** 也可以順手查。
然後才是 **§H（寫 100 題解析）**，做完再跑 1、2、5。

### 不算欠帳的（別再重新討論）

- 階段二（自己從教材生新題）：使用者說暫時不做
- 科目二 `s2`、初級 `e1`/`e2` 的已知品質問題：不在考試範圍，不修（紅線 J6）
...
- [ ] **I6** **把 `--check` 掛成自動執行**（§6.5 計畫寫了，但當初沒排進 checklist，
      ...
      - `.git/hooks/pre-commit` 跑 `--check`，不同步就擋下 commit。⚠️ hook 不進 git，
        clone 後要重裝，所以要在 README/本檔寫明安裝指令
      - 或加 GitHub Actions
