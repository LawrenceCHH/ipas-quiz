...
6. **每台裝置都要設定一次**（網址存在該瀏覽器的 `localStorage`，設定完就不用再貼）。設定好第一台後，按「複製同步連結」，把連結傳給自己的其他裝置（Line/AirDrop/訊息皆可），在那台裝置的瀏覽器開一次連結即可自動套用，不用手動複製貼上網址。之後每台裝置開頁時抓一次、每回合作答完寫回，用每回合的 `id` 去重，不會重複寫入。
...
> 不會經過任何第三方伺服器。
>
> ⚠️ GAS 部署設定是「誰有存取權：所有人」、沒有內建認證，拿到這組網址（或同步連結）
> 的任何人都能讀寫你的試算表。它只存在你貼過的瀏覽器 `localStorage` 裡，**不要**把它
> 寫進任何會進 git 的檔案、或貼在公開頻道。
